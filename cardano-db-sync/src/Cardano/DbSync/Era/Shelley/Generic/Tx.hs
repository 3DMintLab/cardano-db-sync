...
fromAlonzoTx pp (blkIndex, tx) =
    Tx
      ...
      , txOutputs = if not isValid2 then [] else
          zipWith fromTxOut [0 .. ] . toList $ getField @"outputs" txBody
      , txFees = getField @"txfee" txBody
      , txOutSum = if not isValid2 then Coin 0 else
          Coin . sum $ map txOutValue (getField @"outputs" txBody)
